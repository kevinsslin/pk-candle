import { createContext, useCallback, useContext, useEffect, useMemo, useState } from 'react';
import type { PlayerSummary, SessionStatus } from '@pk-candle/shared';

export type Language = 'en' | 'zh-CN';

const STORAGE_KEY = 'pkcandle-lang';

const MESSAGES = {
  en: {
    appName: 'PK Candle',
    turboRoom: 'Turbo Room',
    lobby: 'Lobby',
    leaderboard: 'Leaderboard',
    rankedLeaderboardNav: 'Ranked',
    rankedTitle: 'Solo Ranked',
    rankedSubtitle: 'Verified wallet only · Auto matchmaking',
    rankedVerifyWallet: 'Verify Wallet',
    rankedVerified: 'Verified',
    rankedVerifyHint: 'We only request a signature to verify identity — no transactions.',
    rankedQueueing: 'Queueing... {seconds}s · Range {range}',
    rankedQueueCount: '{count} in queue',
    rankedJoinQueue: 'Join Ranked Queue',
    rankedCancelQueue: 'Cancel Queue',
    rankedConnectWalletFirst: 'Connect your wallet first.',
    rankedVerifyFailed: 'Wallet verification failed.',
    rankedVerifyRequired: 'Verify wallet to queue for ranked.',
    rankedQueueFailed: 'Failed to join ranked queue.',
    rankedSiweStatement: 'This signature is for login only and will not trigger any blockchain transaction.',
    rankedLeaderboardTitle: 'Ranked Leaderboard',
    rankedLeaderboardSubtitle: 'Global rating ladder for Solo Ranked.',
    rankedLeaderboardEmpty: 'No ranked matches yet.',
    rankedLeaderboardLoadFailed: 'Failed to load ranked leaderboard.',
    rankedRatingLabel: 'Rating: {rating}',
    rankedMatchesLabel: '{count} matches',
    rankedResultTitle: 'Ranked Results',
    rankedResultSubtitle: 'Your rating update is based on placement and expected score.',
    rankedYou: 'You',
    adminNav: 'Admin',
    adminTitle: 'Live Metrics',
    adminSubtitle: 'Open dashboard for global game activity.',
    adminUpdatedAt: 'Updated {time}',
    adminSectionLive: 'Live',
    adminSectionTotals: 'Totals',
    adminMetricActivePlayers: 'Active Players',
    adminMetricActiveRooms: 'Active Rooms',
    adminMetricLiveRooms: 'Live Sessions',
    adminMetricRankedRooms: 'Ranked Rooms',
    adminMetricRankedQueue: 'Ranked Queue',
    adminMetricTotalGames: 'Total Games',
    adminMetricRankedMatches: 'Ranked Matches',
    adminMetricRankedParticipants: 'Ranked Participants',
    adminMetricLeaderboardEntries: 'Leaderboard Submissions',
    adminMetricPlayerStarts: 'Player Starts',
    adminMetricsLoadFailed: 'Failed to load metrics.',
    leaveRoom: 'Leave Room',
    signedIn: 'Signed in',
    guest: 'Guest',
    loading: 'Loading',
    login: 'Login',
    logout: 'Logout',
    save: 'Save',
    editName: 'Edit name',
    languageLabel: 'Lang',
    languageToggle: 'Switch language',
    languageFlagsTitle: 'Language',
    languageChinese: '中文（简体）',
    languageEnglish: 'English',
    joiningRoom: 'Checking room access...',

    missingWsTitle: 'Missing WebSocket URL',
    missingWsBody: 'Set VITE_WS_URL to connect to the server.',

    roomNotFoundTitle: 'Room not found',
    roomNotFoundBody: 'Pick a room from the lobby.',
    alreadyInRoomTitle: 'You are already in {roomId}',
    alreadyInRoomBody: 'Leave the current room before joining another.',
    goToRoom: 'Go to Room',
    backToLobby: 'Back to Lobby',
    joinRoomTitle: 'Join Room {roomId}',
    roomLockStatusLocked: 'This room is locked. Enter the room key to join.',
    roomLockStatusOpen: 'This room is open. No room key required.',
    roomLockStatusChecking: 'Checking room lock status. A key may be required.',
    enterRoomKey: 'Enter the room key if it is locked.',
    nameLabel: 'Name',
    nameRequired: 'Enter a name to join.',
    roomKeyLabel: 'Room Key',
    roomKeyOptional: 'Room Key (optional)',
    roomKeyPlaceholder: 'room key',
    roomLockLabel: 'Room Lock',
    roomLockOn: 'Locked',
    roomLockOff: 'Unlocked',
    roomLockHint: 'Require a key to join this room.',
    roomNameLabel: 'Room Name (max 24, emoji ok)',
    roomNamePlaceholder: 'Kevin’s Dinner Table 🤓',
    maxPlayersLabel: 'Max Players (2-6)',
    playersCount: '{count} players',
    enterRoom: 'Enter Room',
    connecting: 'Connecting...',

    sessionTitle: 'Session',
    statusLabel: 'Status',
    roundLabel: 'Round {round}/{total}',
    timeLeftLabel: 'Time left {time}',
    startingIn: 'Starting in {seconds}s',
    getReady: 'Get ready',
    paused: 'Paused',
    eventPausedTitle: 'Global event pause',
    eventPausedBody: 'Market event resolved. Trading resumes after the countdown.',
    eventImpactTitle: 'Impact',
    eventImpactPhase: 'Phase: {phase}',
    eventImpactVolatility: 'Volatility: {value}',
    eventImpactPrice: 'Price multiplier: x{value}',
    eventImpactNeutral: 'No direct market impact.',
    nextRoundIn: 'Next round in {seconds}s',
    personalEventCountdown: 'Auto-resolve in {seconds}s',
    personalEventExpired: 'Event expired. Resolving...',
    respawnWarning: 'System Alert',
    respawnTitleBroke: 'Liquidated',
    respawnTitleDead: 'Knocked Out',
    respawnPenalty: 'Penalty: reset to {percent}% of base cash.',
    respawnCurrentCash: 'Current cash: {cash} U',
    respawnNewCash: 'New cash: {cash} U',
    respawnCountdown: 'Respawn in {seconds}s',

    you: 'You',
    cash: 'Cash',
    equity: 'Equity',
    pnl: 'PnL',
    stress: 'Stress',
    connection: 'Connection',
    connectionIdle: 'Idle',
    connectionConnecting: 'Connecting',
    connectionConnected: 'Connected',
    connectionError: 'Error',

    globalEvents: 'Global Events',
    noMarketShocks: 'No market shocks yet.',

    marketTitle: 'Market',
    loadingToken: 'Loading token...',
    phaseLabel: 'Phase: {phase}',
    goLive: 'Go Live',
    fitView: 'Fit View',
    localTime: 'Local time: {time}',
    lastCandle: 'Last candle: {time}',
    scrollToZoom: 'Scroll to zoom',
    timeframe1s: '1s',
    timeframe5s: '5s',
    timeframe10s: '10s',

    tradeTitle: 'Trade',
    tradingDisabledTitle: 'Trading disabled',
    tradingDisabledGeneric: 'You cannot trade right now.',
    orderSize: 'Order Size',
    percent: 'Percent',
    amount: 'Amount',
    available: 'Available',
    orderValue: 'Order value',
    size: 'Size',
    positionMargin: 'Position margin',
    addOrderValue: 'Add value',
    reduceOrderValue: 'Reduce value',
    tpPlaceholder: 'TP %',
    slPlaceholder: 'SL %',
    tpSlTitle: 'Optional TP/SL',
    tpSlHint: 'Auto-close when price hits your target.',
    leverage: 'Leverage',
    long: 'Long',
    short: 'Short',
    openLong: 'Open Long',
    openShort: 'Open Short',
    addLong: 'Add Long',
    addShort: 'Add Short',
    reduceLong: 'Reduce Long',
    reduceShort: 'Reduce Short',
    closePosition: 'Close Position',
    positionTitle: 'Position',
    noOpenPosition: 'No open position.',
    side: 'Side',
    entry: 'Entry',
    price: 'Price',
    liquidation: 'Liquidation',
    entryLine: 'Entry',
    avgCostLine: 'Avg Cost',
    markPriceLine: 'Mark Price',
    liquidationLine: 'Liq',
    margin: 'Margin',
    tradeHistory: 'Recent Trades',
    tradeOpen: 'Open',
    tradeClose: 'Close',
    tradeLiquidation: 'Liquidation',
    noTradesYet: 'No trades yet.',

    chat: 'Chat',
    chatRoom: 'Chat Room',
    chatPlaceholder: 'Type a message',
    chatJoinPlaceholder: 'Join the room to chat',
    send: 'Send',
    danmakuOn: 'On',
    danmakuOff: 'Off',
    danmakuLabel: 'Danmaku',
    close: 'Close',
    spectatorTag: 'SPEC',

    roomRank: 'Room Rank',
    topN: 'Top {count}',
    roomPnl: 'Room PnL',
    noPlayersYet: 'No players yet.',
    packLabel: 'Pack',
    corePack: 'Core',
    hostLabel: 'Host',
    packHelp: 'Event pack = room rules + events.',
    packTitle: 'Event Pack',
    packSelectLabel: 'Select pack',
    packVersion: 'v{version}',
    packNoDescription: 'No description yet.',
    packPersonalCount: '{count} Personal',
    packMarketCount: '{count} Market',
    createPack: 'Create Pack',
    editPack: 'Edit Pack',
    deletePack: 'Delete Pack',
    packLockedNote: 'Pack changes are locked after countdown.',

    lobbyTitle: 'Lobby',
    roomLabel: 'Room: {roomId}',
    inviteCrew: 'Invite Crew',
    inviteCopied: 'Invite Copied',
    inviteFailed: 'Copy Failed',
    inviteTitle: 'PK Candle raid is live.',
    inviteRoomCode: 'Room code: {roomId}',
    inviteBringTrades: 'Bring your best trades.',
    inviteLockedNote: 'Room is locked. Ask host for the key.',
    inviteUnlockedNote: 'No key required.',
    inviteJoin: 'Join: {url}',
    inviteCopyPrompt: 'Copy this invite',
    readyCount: 'Ready {ready}/{total}',
    cancelReady: 'Cancel Ready',
    readyUp: 'Ready Up',
    startCountdown: 'Start Countdown',
    waitingHost: 'Waiting for host',
    readyUpToStart: 'Ready up to start',
    roomLockTitle: 'Room Lock',
    locked: 'Locked',
    unlocked: 'Unlocked',
    hostControlsKey: 'Host controls the room key.',
    setUpdateKey: 'Set / Update Key',
    setRoomKeyPlaceholder: 'set room key',
    setNewRoomKeyPlaceholder: 'set a new room key',
    updateKey: 'Update Key',
    removeLock: 'Remove Lock',
    lockHelp: 'Players enter the key after entering the room.',
    lockDisabled: 'Lock changes are disabled once the countdown starts.',
    seatsTitle: 'Seats (max {max})',
    seatLabel: 'Seat {number}',
    ready: 'Ready',
    notReady: 'Not ready',
    online: 'Online',
    offline: 'Offline',
    kick: 'Kick',
    emptySeat: 'Empty seat',
    spectatorsTitle: 'Spectators',
    spectatorDisabled: 'Spectator mode is disabled for now.',
    hostTag: 'HOST',

    startTitle: 'PK Candle',
    startTagline: 'Meme-market PvP. Ride the same candle, survive the chaos, and flex your gains.',
    startBadgeLive: 'Live Rooms',
    startBadgeLobbies: '6-Player Lobbies',
    startBadgeLeaderboard: 'Global Leaderboard',

    createRoomTitle: 'Create Room',
    createRoomBody: 'Spin up a lobby, then share the invite from inside the room.',
    playerNameLabel: 'Player Name',
    playerNamePlaceholder: 'Turbo Trader',
    roomCodeLabel: 'Room Code',
    roomCodeAutoPlaceholder: 'auto code',
    newCode: 'New Code',
    roomNameLabelShort: 'Room Name (max 24)',
    createRoom: 'Create Room',

    howToPlay: 'Rules',
    open: 'Open',
    advanced: 'Advanced',
    done: 'Done',

    quickJoin: 'Quick Join',
    quickJoinBody: 'Paste a room code or full invite link. We will auto-detect it.',
    roomCodePlaceholder: 'Room code or invite URL',
    joinRoom: 'Join Room',
    lockedNote: 'If the room is locked, you will enter the key after joining.',

    activeRooms: 'Active Rooms',
    refresh: 'Refresh',
    searchRoomPlaceholder: 'Search room code or host',
    noPublicRooms: 'No public rooms yet. Create one!',
    noRoomsMatch: 'No rooms match your search.',
    full: 'Full',
    join: 'Join',
    playersOnline: '{current}/{max} players',

    leaderboardTitle: 'Global Leaderboard',
    leaderboardSubtitle: 'Top ROI, peak cash, and bragging rights.',
    share: 'Share',
    leaderboardShareHeadline: '{name} just hit {roi}% ROI in PK Candle.',
    leaderboardShareFallback: 'PK Candle global leaderboard is live.',
    leaderboardShareBody: '{headline} Come claim your spot.',
    leaderboardNameDefault: 'Gamerun',
    leaderboardNameLabel: 'Leaderboard name',
    leaderboardNamePlaceholder: 'Enter your public name',
    leaderboardNameHint: 'This name will be shown publicly. You can edit it before submitting.',
    leaderboardConfirmSubmit: 'Submit to the leaderboard as “{name}”?',
    localHistoryTitle: 'Your recent runs',
    localHistoryEmpty: 'No local runs yet.',
    localHistoryCash: 'Cash {cash} U',
    localHistoryRoi: 'ROI {roi}%',
    localHistoryRound: 'Round {round}',
    noScoresYet: 'No scores yet.',
    peak: 'Peak',
    pageNotFoundTitle: 'Page not found',
    pageNotFoundBody: 'Head back to the lobby.',
    goHome: 'Go Home',
    loginToAppear: 'Log in to appear on leaderboard',
    claimLeaderboard: 'Claim Leaderboard',

    gameOver: 'Game Over',
    youGotRekt: 'You got rekt.',
    sessionEnded: 'Session ended.',
    finalCash: 'Final Cash',
    shareToTwitter: 'Share to Twitter',
    topPlayers: 'Top Players',
    gameOverShare: 'Just survived PK Candle with ROI {roi}%! Join the chaos: {url}',
    cancel: 'Cancel',
    savePack: 'Save Pack',
    packCreateTitle: 'Create Event Pack',
    packEditTitle: 'Edit Event Pack',
    packEditorBody: 'Edit the JSON and save. This pack can be used as a map for your room.',
    packInvalidJson: 'Invalid JSON. Make sure the pack format is valid.',

    tradingDisabledJoin: 'Join the room to trade.',
    tradingDisabledNotActive: 'Trading disabled: player not ACTIVE.',
    tradingDisabledNotLive: 'Trading disabled: session is not LIVE.',
    respawnPaused: 'Respawning... {seconds}s',
    tradingDisabledPersonalEvent: 'Trading paused: personal event.',
    tradingDisabledPaused: 'Trading paused.',
    tradingDisabledInvalidPrice: 'Trading disabled: invalid market price.',
    authFailed: 'Failed to authenticate.',
    wsError: 'WebSocket error',
    leaderboardLoadFailed: 'Failed to load leaderboard.',
    leaderboardSubmitting: 'Submitting leaderboard...',
    leaderboardSubmitted: 'Leaderboard submitted.',
    leaderboardSubmitTimeout: 'Leaderboard submit timed out. Please try again.',
    leaderboardYourRank: 'Your rank: #{rank}',
    paginationPrev: 'Prev',
    paginationNext: 'Next',
    pageOf: 'Page {page} / {total}',
    leaderboardAlreadySubmitted: 'Already submitted.',
    leaderboardSubmitFailed: 'Failed to submit leaderboard.',
    leaderboardNoResult: 'No finished run found. Finish a match first.',
    leaderboardDbMissing: 'Leaderboard database not configured.',
    leaderboardDbUnavailable: 'Leaderboard database unavailable. Try again later.',
    leaderboardDbSchemaMissing: 'Leaderboard tables missing. Run db:push.',
    leaderboardServiceUnavailable: 'Leaderboard service unavailable. Try again later.',
    leaderboardPrivyMissing: 'Privy server keys are missing.',
    leaderboardWsDisconnected: 'Not connected to the game server.',
    chartLoadFailed: 'Chart failed to load',
    chartLoadFailedHint: 'Please refresh the page or try again.',
    reload: 'Reload',
  },
  'zh-CN': {
    appName: 'PK Candle',
    turboRoom: '极速房',
    lobby: '大厅',
    leaderboard: '排行榜',
    rankedLeaderboardNav: '排位榜',
    rankedTitle: '单人排位',
    rankedSubtitle: '仅限钱包验证 · 自动匹配',
    rankedVerifyWallet: '验证钱包',
    rankedVerified: '已验证',
    rankedVerifyHint: '仅签名验证身份，不会发起任何交易。',
    rankedQueueing: '匹配中... {seconds}s · 匹配范围 {range}',
    rankedQueueCount: '排队中 {count} 人',
    rankedJoinQueue: '开始匹配',
    rankedCancelQueue: '取消匹配',
    rankedConnectWalletFirst: '请先连接钱包。',
    rankedVerifyFailed: '钱包验证失败。',
    rankedVerifyRequired: '需要验证钱包才能排位。',
    rankedQueueFailed: '排位匹配失败。',
    rankedSiweStatement: '此签名仅用于登录验证，不会触发任何链上交易。',
    rankedLeaderboardTitle: '排位排行榜',
    rankedLeaderboardSubtitle: '全球单人排位分数榜。',
    rankedLeaderboardEmpty: '暂时没有排位数据。',
    rankedLeaderboardLoadFailed: '加载排位排行榜失败。',
    rankedRatingLabel: '评分：{rating}',
    rankedMatchesLabel: '{count} 场对局',
    rankedResultTitle: '排位结算',
    rankedResultSubtitle: '分数根据名次与预期值结算。',
    rankedYou: '你',
    adminNav: '数据看板',
    adminTitle: '实时数据',
    adminSubtitle: '面向所有玩家开放的游戏数据面板。',
    adminUpdatedAt: '更新时间 {time}',
    adminSectionLive: '实时',
    adminSectionTotals: '累计',
    adminMetricActivePlayers: '在线玩家',
    adminMetricActiveRooms: '活跃房间',
    adminMetricLiveRooms: '进行中对局',
    adminMetricRankedRooms: '排位房间',
    adminMetricRankedQueue: '排位排队人数',
    adminMetricTotalGames: '累计对局',
    adminMetricRankedMatches: '排位对局',
    adminMetricRankedParticipants: '排位参赛人次',
    adminMetricLeaderboardEntries: '排行榜提交次数',
    adminMetricPlayerStarts: '玩家开局次数',
    adminMetricsLoadFailed: '加载数据失败。',
    leaveRoom: '离开房间',
    signedIn: '已登录',
    guest: '游客',
    loading: '加载中',
    login: '登录',
    logout: '登出',
    save: '保存',
    editName: '修改昵称',
    languageLabel: '语言',
    languageToggle: '切换语言',
    languageFlagsTitle: '语言',
    languageChinese: '中文（简体）',
    languageEnglish: '英文',
    joiningRoom: '正在检查房间权限...',

    missingWsTitle: '缺少 WebSocket 地址',
    missingWsBody: '请设置 VITE_WS_URL 以连接服务器。',

    roomNotFoundTitle: '房间不存在',
    roomNotFoundBody: '请从大厅选择房间。',
    alreadyInRoomTitle: '你已在房间 {roomId}',
    alreadyInRoomBody: '加入其他房间前请先离开当前房间。',
    goToRoom: '进入房间',
    backToLobby: '返回大厅',
    joinRoomTitle: '加入房间 {roomId}',
    roomLockStatusLocked: '该房间已上锁，请输入房间钥匙后加入。',
    roomLockStatusOpen: '该房间未上锁，无需钥匙即可进入。',
    roomLockStatusChecking: '正在检查房间锁定状态，可能需要钥匙。',
    enterRoomKey: '若房间上锁，请输入房间钥匙。',
    nameLabel: '名字',
    nameRequired: '请输入昵称后加入。',
    roomKeyLabel: '房间钥匙',
    roomKeyOptional: '房间钥匙（可选）',
    roomKeyPlaceholder: '房间钥匙',
    roomLockLabel: '房间上锁',
    roomLockOn: '已上锁',
    roomLockOff: '未上锁',
    roomLockHint: '开启后需要钥匙才能进入。',
    roomNameLabel: '房间名称（最多 24 字，支持表情）',
    roomNamePlaceholder: 'Kevin 的晚餐桌 🤓',
    maxPlayersLabel: '人数上限（2-6）',
    playersCount: '{count} 人',
    enterRoom: '进入房间',
    connecting: '连接中…',

    sessionTitle: '回合',
    statusLabel: '状态',
    roundLabel: '回合 {round}/{total}',
    timeLeftLabel: '剩余时间 {time}',
    startingIn: '即将开始 {seconds}s',
    getReady: '准备开始',
    paused: '暂停中',
    eventPausedTitle: '全局事件暂停',
    eventPausedBody: '市场事件结算中，倒计时结束后继续交易。',
    eventImpactTitle: '影响说明',
    eventImpactPhase: '阶段：{phase}',
    eventImpactVolatility: '波动：{value}',
    eventImpactPrice: '价格倍数：x{value}',
    eventImpactNeutral: '暂无直接市场影响。',
    nextRoundIn: '下一回合 {seconds}s',
    personalEventCountdown: '倒计时 {seconds}s',
    personalEventExpired: '事件已超时，正在结算...',
    respawnWarning: '系统提示',
    respawnTitleBroke: '爆仓了',
    respawnTitleDead: '倒下了',
    respawnPenalty: '惩罚：资金重置为基础资金的 {percent}%。',
    respawnCurrentCash: '当前资金：{cash} U',
    respawnNewCash: '重新发放：{cash} U',
    respawnCountdown: '重新进入倒计时 {seconds}s',

    you: '你',
    cash: '现金',
    equity: '权益',
    pnl: '盈亏',
    stress: '压力',
    connection: '连接',
    connectionIdle: '空闲',
    connectionConnecting: '连接中',
    connectionConnected: '已连接',
    connectionError: '错误',

    globalEvents: '全局事件',
    noMarketShocks: '暂无市场冲击。',

    marketTitle: '市场',
    loadingToken: '载入币种…',
    phaseLabel: '阶段：{phase}',
    goLive: '回到最新',
    fitView: '适配视图',
    localTime: '本地时间：{time}',
    lastCandle: '最新 K 线：{time}',
    scrollToZoom: '滚轮缩放',
    timeframe1s: '1 秒',
    timeframe5s: '5 秒',
    timeframe10s: '10 秒',

    tradeTitle: '交易',
    tradingDisabledTitle: '交易不可用',
    tradingDisabledGeneric: '当前无法交易。',
    orderSize: '下单比例',
    percent: '比例',
    amount: '金额',
    available: '可用',
    orderValue: '下单金额',
    size: '数量',
    positionMargin: '持仓保证金',
    addOrderValue: '加仓金额',
    reduceOrderValue: '减仓金额',
    tpPlaceholder: '止盈 %',
    slPlaceholder: '止损 %',
    tpSlTitle: '止盈止损（可选）',
    tpSlHint: '触发目标价后自动平仓。',
    leverage: '杠杆',
    long: '做多',
    short: '做空',
    openLong: '开多',
    openShort: '开空',
    addLong: '加多',
    addShort: '加空',
    reduceLong: '减多',
    reduceShort: '减空',
    closePosition: '平仓',
    positionTitle: '持仓',
    noOpenPosition: '暂无持仓。',
    side: '方向',
    entry: '开仓价',
    price: '价格',
    liquidation: '强平价',
    entryLine: '开仓',
    avgCostLine: '平均成本',
    markPriceLine: '现价',
    liquidationLine: '强平',
    margin: '保证金',
    tradeHistory: '交易记录',
    tradeOpen: '开仓',
    tradeClose: '平仓',
    tradeLiquidation: '爆仓',
    noTradesYet: '暂无交易记录。',

    chat: '聊天',
    chatRoom: '聊天室',
    chatPlaceholder: '输入消息',
    chatJoinPlaceholder: '加入房间后才能聊天',
    send: '发送',
    danmakuOn: '开',
    danmakuOff: '关',
    danmakuLabel: '弹幕',
    close: '关闭',
    spectatorTag: '观战',

    roomRank: '房内排行',
    topN: '前 {count}',
    noPlayersYet: '暂无玩家。',
    roomPnl: '房内盈亏',
    packLabel: '事件包',
    corePack: '核心包',
    hostLabel: '房主',
    packHelp: '事件包 = 房间规则与事件。',
    packTitle: '事件包',
    packSelectLabel: '选择事件包',
    packVersion: 'v{version}',
    packNoDescription: '暂无说明。',
    packPersonalCount: '个人事件 {count}',
    packMarketCount: '全局事件 {count}',
    createPack: '新建事件包',
    editPack: '编辑事件包',
    deletePack: '删除事件包',
    packLockedNote: '倒计时开始后无法切换事件包。',

    lobbyTitle: '大厅',
    roomLabel: '房间：{roomId}',
    inviteCrew: '邀请队友',
    inviteCopied: '已复制邀请',
    inviteFailed: '复制失败',
    inviteTitle: 'PK Candle 车队出发。',
    inviteRoomCode: '房间码：{roomId}',
    inviteBringTrades: '带上你的最佳交易。',
    inviteJoin: '加入：{url}',
    inviteLockedNote: '房间已上锁，请向房主索取钥匙。',
    inviteUnlockedNote: '无需钥匙即可进入。',
    inviteCopyPrompt: '复制邀请内容',
    readyCount: '准备 {ready}/{total}',
    cancelReady: '取消准备',
    readyUp: '准备好了',
    startCountdown: '开始倒计时',
    waitingHost: '等待房主',
    readyUpToStart: '所有人准备后开始',
    roomLockTitle: '房间锁',
    locked: '已上锁',
    unlocked: '未上锁',
    hostControlsKey: '只有房主能修改钥匙。',
    setUpdateKey: '设置 / 更新钥匙',
    setRoomKeyPlaceholder: '设置房间钥匙',
    setNewRoomKeyPlaceholder: '设置新的房间钥匙',
    updateKey: '更新钥匙',
    removeLock: '移除锁',
    lockHelp: '玩家进入房间后再输入钥匙。',
    lockDisabled: '倒计时开始后无法更改。',
    seatsTitle: '座位（最多 {max}）',
    seatLabel: '座位 {number}',
    ready: '已准备',
    notReady: '未准备',
    online: '在线',
    offline: '离线',
    kick: '踢出',
    emptySeat: '空位',
    spectatorsTitle: '观众',
    spectatorDisabled: '暂不开放观战模式。',
    hostTag: '房主',

    startTitle: 'PK Candle',
    startTagline: '迷因市场对战。同一根 K 线一起冲，活到最后，秀出收益。',
    startBadgeLive: '实时房间',
    startBadgeLobbies: '6 人大厅',
    startBadgeLeaderboard: '全球排行榜',

    createRoomTitle: '创建房间',
    createRoomBody: '创建房间后，可在房内复制邀请链接。',
    playerNameLabel: '玩家名称',
    playerNamePlaceholder: '极速交易员',
    roomCodeLabel: '房间代码',
    roomCodeAutoPlaceholder: '自动生成',
    newCode: '新代码',
    roomNameLabelShort: '房间名称（最多 24 字）',
    createRoom: '创建房间',

    howToPlay: '规则说明',
    open: '打开',
    advanced: '高级',
    done: '完成',

    quickJoin: '快速加入',
    quickJoinBody: '粘贴房间代码或完整邀请链接，我们会自动识别。',
    roomCodePlaceholder: '房间代码或邀请链接',
    joinRoom: '加入房间',
    lockedNote: '如果房间上锁，加入后会提示输入钥匙。',

    activeRooms: '活跃房间',
    refresh: '刷新',
    searchRoomPlaceholder: '搜索房间码或房主',
    noPublicRooms: '还没有公开房间，快创建一个！',
    noRoomsMatch: '没有符合条件的房间。',
    full: '已满',
    join: '加入',
    playersOnline: '{current}/{max} 人',

    leaderboardTitle: '全球排行榜',
    leaderboardSubtitle: '最高 ROI、峰值资金和炫耀权。',
    share: '分享',
    leaderboardShareHeadline: '{name} 在 PK Candle 达成 {roi}% ROI！',
    leaderboardShareFallback: 'PK Candle 全球排行榜已开启。',
    leaderboardShareBody: '{headline} 快来占位！',
    leaderboardNameDefault: 'Gamerun',
    leaderboardNameLabel: '排行榜昵称',
    leaderboardNamePlaceholder: '填写展示昵称',
    leaderboardNameHint: '昵称会公开展示，你可以在提交前修改。',
    leaderboardConfirmSubmit: '确定以「{name}」提交排行榜？',
    localHistoryTitle: '本地战绩',
    localHistoryEmpty: '暂无本地战绩。',
    localHistoryCash: '现金 {cash} U',
    localHistoryRoi: 'ROI {roi}%',
    localHistoryRound: '第 {round} 回合',
    noScoresYet: '暂无成绩。',
    peak: '峰值',
    pageNotFoundTitle: '页面不存在',
    pageNotFoundBody: '返回大厅继续操作。',
    goHome: '返回首页',
    loginToAppear: '登录后才会出现在排行榜',
    claimLeaderboard: '领取排行榜',

    gameOver: '游戏结束',
    youGotRekt: '你被清算了。',
    sessionEnded: '本回合结束。',
    finalCash: '最终现金',
    shareToTwitter: '分享至推特',
    topPlayers: '高手榜',
    gameOverShare: '刚在 PK Candle 活到最后，ROI {roi}%！加入战局：{url}',
    cancel: '取消',
    savePack: '保存包',
    packCreateTitle: '创建事件包',
    packEditTitle: '编辑事件包',
    packEditorBody: '编辑 JSON 并保存。该包可作为房间地图使用。',
    packInvalidJson: 'JSON 无效，请确认格式正确。',

    tradingDisabledJoin: '加入房间后才能交易。',
    tradingDisabledNotActive: '交易不可用：玩家状态非 ACTIVE。',
    tradingDisabledNotLive: '交易不可用：回合未开始。',
    respawnPaused: '爆仓保护中... {seconds}s',
    tradingDisabledPersonalEvent: '个人事件处理中，暂停交易。',
    tradingDisabledPaused: '交易已暂停。',
    tradingDisabledInvalidPrice: '交易不可用：价格无效。',
    authFailed: '认证失败。',
    wsError: 'WebSocket 错误',
    leaderboardLoadFailed: '加载排行榜失败。',
    leaderboardSubmitting: '正在提交排行榜...',
    leaderboardSubmitted: '排行榜提交成功。',
    leaderboardSubmitTimeout: '提交超时，请重试。',
    leaderboardYourRank: '你的排名：#{rank}',
    paginationPrev: '上一页',
    paginationNext: '下一页',
    pageOf: '第 {page}/{total} 页',
    leaderboardAlreadySubmitted: '已提交过。',
    leaderboardSubmitFailed: '提交排行榜失败。',
    leaderboardNoResult: '找不到结束战绩，请先完成一局。',
    leaderboardDbMissing: '排行榜数据库尚未配置。',
    leaderboardDbUnavailable: '排行榜数据库不可用，请稍后再试。',
    leaderboardDbSchemaMissing: '排行榜表不存在，请先执行 db:push。',
    leaderboardServiceUnavailable: '排行榜服务不可用，请稍后再试。',
    leaderboardPrivyMissing: '服务器未配置 Privy。',
    leaderboardWsDisconnected: '未连接到游戏服务器。',
    chartLoadFailed: '图表加载失败',
    chartLoadFailedHint: '请刷新页面或稍后再试。',
    reload: '刷新',
  },
};

const STATUS_LABELS: Record<Language, Record<SessionStatus, string>> = {
  en: {
    LOBBY: 'Lobby',
    COUNTDOWN: 'Countdown',
    LIVE: 'Live',
    ENDED: 'Ended',
  },
  'zh-CN': {
    LOBBY: '大厅',
    COUNTDOWN: '倒计时',
    LIVE: '进行中',
    ENDED: '已结束',
  },
};

const PLAYER_STATUS_LABELS: Record<Language, Record<PlayerSummary['status'], string>> = {
  en: {
    ACTIVE: 'LIVE',
    ELIMINATED: 'OUT',
    FINISHED: 'DONE',
  },
  'zh-CN': {
    ACTIVE: '进行中',
    ELIMINATED: '淘汰',
    FINISHED: '结束',
  },
};

const I18nContext = createContext<{
  lang: Language;
  setLang: (next: Language) => void;
  t: (key: keyof typeof MESSAGES.en, vars?: Record<string, string | number>) => string;
} | null>(null);

const interpolate = (template: string, vars?: Record<string, string | number>) => {
  if (!vars) return template;
  return template.replace(/\{(\w+)\}/g, (_, key) => {
    const value = vars[key];
    return value === undefined ? '' : String(value);
  });
};

export const I18nProvider = ({ children }: { children: React.ReactNode }) => {
  const [lang, setLangState] = useState<Language>(() => {
    if (typeof window === 'undefined') return 'zh-CN';
    const stored = window.localStorage.getItem(STORAGE_KEY) as Language | null;
    return stored === 'en' || stored === 'zh-CN' ? stored : 'zh-CN';
  });

  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem(STORAGE_KEY, lang);
  }, [lang]);

  const setLang = useCallback((next: Language) => {
    setLangState(next);
  }, []);

  const t = useCallback((key: keyof typeof MESSAGES.en, vars?: Record<string, string | number>) => {
    const bundle = MESSAGES[lang] ?? MESSAGES.en;
    const template = bundle[key] ?? MESSAGES.en[key] ?? key;
    return interpolate(String(template), vars);
  }, [lang]);

  const value = useMemo(() => ({ lang, setLang, t }), [lang, setLang, t]);

  return (
    <I18nContext.Provider value={value}>
      {children}
    </I18nContext.Provider>
  );
};

export const useI18n = () => {
  const context = useContext(I18nContext);
  if (!context) throw new Error('useI18n must be used within I18nProvider');
  return context;
};

export const formatSessionStatus = (status: SessionStatus, lang: Language) => {
  return STATUS_LABELS[lang][status] ?? status;
};

export const formatPlayerStatus = (status: PlayerSummary['status'], lang: Language) => {
  return PLAYER_STATUS_LABELS[lang][status] ?? status;
};

export const getHowToPlaySections = (lang: Language) => {
  if (lang === 'zh-CN') {
    return [
      {
        title: '目标',
        body: '在 3 分钟回合内生存，拿到最高 ROI，并登上全球排行榜。',
      },
      {
        title: '资金与仓位',
        body: '使用滑杆投入 5–100% 资金（默认 1x 杠杆）。平仓即可锁定盈亏；价格触及强平线会被清算。',
      },
      {
        title: '全局事件',
        body: '市场冲击会影响所有人，并短暂暂停交易。所有人看到同一根 K 线。',
      },
      {
        title: '个人事件',
        body: '只对你可见，选择选项会改变现金（与压力提示）。',
      },
      {
        title: '房间流程',
        body: '所有人准备后由房主开始 5 秒倒计时。任一人取消准备则倒计时停止。',
      },
      {
        title: '回合',
        body: '3 分钟回合分为 6 轮（进度条显示）。爆仓后会重生并重置资金。',
      },
    ];
  }

  return [
    {
      title: 'Goal',
      body: 'Survive the 3-minute session, stack the highest ROI, and claim the global leaderboard.',
    },
    {
      title: 'Cash & Positions',
      body: 'Use the size slider to commit 5–100% of your cash per trade (1x leverage). Close to lock PnL. Liquidation hits when price crosses your liq line.',
    },
      {
        title: 'Global Events',
        body: 'Market shocks broadcast to everyone and pause trading for a few seconds. Everyone sees the same candle move.',
      },
      {
        title: 'Personal Events',
        body: 'Only you see your personal event. Pick a choice to adjust cash (plus stress flavor).',
      },
      {
        title: 'Room Flow',
      body: 'Everyone readies up. Host starts a 5-second countdown. If anyone cancels ready, the countdown stops.',
    },
    {
      title: 'Rounds',
      body: 'The 3-minute session is split into 6 rounds (progress bar). If you blow up, you respawn with fresh funds instead of being kicked.',
    },
  ];
};
